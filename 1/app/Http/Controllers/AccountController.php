    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            ...
            return DataTables::of($accounts)
                ->addColumn('action', function ($row) use ($account_access) {
                    $html = '';
                    $disabled = null;
        ...
        return view('account.index')
            ->with(compact('not_linked_payments', 'account_types', 'account_access', 'business_locations', 'account_groups', 'users', 'accounts','suppliers','orderStatuses'));
    }
    ...
    public function show($id)
    {
        ...
        if (request()->ajax()) {
            ...
            return DataTables::of($accounts)
                ...
                ->editColumn('reconcile_status', function ($row) use ($id) {
                    $html = '';
                    if (auth()->user()->can('account.reconcile')) {
                        if ($row->reconcile_status == 0) {
                            $html = '<button type="button" class="btn btn-xs reconcile_status_btn" style="background: #FEA61E; color:#fff;" data-href="' . action('AccountController@reconcile', [$id]) . '"><i class="fa fa-times"></i> ' . __('account.reconcile') . '</button>';
                        } else {
                            $html = '<button type="button" class="btn btn-xs reconcile_status_btn" style="background: #3AEC05; color:#fff;" data-href="' . action('AccountController@reconcile', [$id]) . '"><i class="fa fa-check"></i> ' . __('account.reconciled') . '</button>';
                        }
                    }
                    return $html;
                })
