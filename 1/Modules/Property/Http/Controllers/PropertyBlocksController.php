...
class PropertyBlocksController extends Controller
{
    ...
    public function getBlockTr($id)
    {
        ...
        if (!empty(request()->user_id)) {
            $query->where('property_blocks.sold_by', request()->user_id);
        }
        if (!empty(request()->customer_id)) {
            $query->where('property_blocks.customer_id', request()->customer_id);
        }
        if (!empty(request()->block_id)) {
            $query->where('property_blocks.id', request()->block_id);
        }
