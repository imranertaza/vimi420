...

{
...

    {
        ...
        //     $query = DipReading::leftjoin('business_locations', 'dip_readings.location_id', 'business_locations.id')
        ...
        //         $query->where('dip_readings.tank_id', request()->tank_id);
        ...
        //         $query->whereBetween('dip_readings.date_and_time', [date(request()->start_date), date(request()->end_date)]);
        ...
        //         ->editColumn('current_qty', function ($row) use ($business_details) {
        ...
        if (request()->ajax()) {
            ...
            $dip_report = Datatables::of($query)
                ...
                ->addColumn('difference_value', function ($row) use ($business_details) {

                    return $this->productUtil->num_f(($row->fuel_balance_dip_reading - $row->current_qty) * $row->min_price, false, $business_details, true);
                })
            ...
            return $dip_report->rawColumns(['difference', 'difference_value'])
...

    {
        ...
        try {
            ...
            $data = array(
                ...
                'transaction_date' => Carbon::parse($request->date_and_time)->format('Y-m-d'),
...

    {
        ...
        if (request()->ajax()) {
            ...
            $query = DipResetting::leftjoin('business_locations', 'dip_resettings.location_id', 'business_locations.id')
            ...
            $dip_report = Datatables::of($query)
                //     ->editColumn('reset_new_dip', function ($row){
...

    {
        ...
        $input_data['transaction_date'] = Carbon::parse($request->date_and_time)->format('Y-m-d H:i:s');
        ...
        $quantity = $request->reset_new_dip - $request->current_qty;
...

    {
        ...
        $current_diff    = $this->transactionUtil->getTankCurrentDifference($tank_id);
        ...
        return ['current_stock' => number_format($current_balance, 3, '.', ''), 'details' => $details, 'dip_readings' =>  $tank_dip_chart, 'product' => $product, 'currenct_diff' => number_format($current_diff, 3, '.', '')];
    }
}
