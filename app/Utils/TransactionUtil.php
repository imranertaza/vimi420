...

{
...

    {
        ...
        return ($purchase_qty  - $sell_qty) +  $stock_adjusted;
...

    {
        ...
        $balance = $purchase_query->pruchase_qty - $sell_query->sell_qty + $stock_adjusted;
    ...
    public function getProductDropDownArray($business_id)

    {
        ...
        $products = Product::join('units', 'products.unit_id', 'units.id')
            ...
            ->select('products.id', 'products.name', 'products.sku', 'units.short_name as unit', 'variation_location_details.qty_available as qty', 'variations.dpp_inc_tax as price')->get();

        $product_arr = [];

        foreach ($products as $product) {

            $product_arr[$product->id] = $product->sku . ', ' . $product->name . ', ' . $this->num_f($product->qty, false, $business_details, true) . ' ' . $product->unit . ', ' . $this->num_f($product->price, true, $business_details, false);
